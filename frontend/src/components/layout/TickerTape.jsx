...
const FALLBACK_DATA = [
  { symbol: 'S&P 500', price: 4500.25, change: +15.3 },
  { symbol: 'AAPL', price: 150.75, change: -1.2 },
  { symbol: 'MSFT', price: 300.1, change: +2.5 },
  { symbol: 'AMZN', price: 3500.5, change: -10.0 },
  { symbol: 'NVDA', price: 220.4, change: +5.0 },
  { symbol: 'GOOGL', price: 2800.0, change: -20.0 },
  { symbol: 'META', price: 330.25, change: +3.75 },
  { symbol: 'TSLA', price: 700.0, change: -15.0 },
  { symbol: 'JPM', price: 160.5, change: +1.5 },
];
...
function TickerTape() {
  ...
  return (
    <div className="overflow-hidden bg-[var(--color-ticker-bg)] border-b border-[var(--color-border)] py-2">
      <div
        className="flex animate-ticker w-max hover:[animation-play-state:paused]"
        style={{ height: '28px' }}
      >
        {tickerData.concat(tickerData).map((item, index) => (
          <div
            key={index}
            className="flex items-center gap-1 px-7 text-sm font-medium tracking-wide border-r border-border whitespace-nowrap"
          >
            <span className="text-muted font-light">{item.symbol}</span>
            <span className="text-ink">{item.price !== null ? item.price.toFixed(2) : '--'}</span>
