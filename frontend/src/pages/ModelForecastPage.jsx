...
export default function ModelForecastPage() {
  ...
  const meta = MODEL_META[modelName] ?? {
    fullName: modelName.toUpperCase(), category: 'Model', complexity: 0, description: '',
  };
  ...
  const convertedClose = latest ? (parseFloat(latest.predicted_close) * rate).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : null;
    ...
    <div className="mx-auto px-8 py-12" style={{ maxWidth: '1100px' }}>
      ...
      <div className="mb-8">
        ...
        <h1
          ...
          style={{ fontSize: 'clamp(2.6rem, 5vw, 3.8rem)', color: 'var(--color-ink)', lineHeight: 1.05, fontWeight: 700 }}
        >
          {meta.fullName}
        </h1>
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          {latest && (
            ...
            >
              <div>
                <p className="text-[0.68rem] font-medium tracking-[0.1em] uppercase mb-1"
                  style={{ color: 'var(--color-muted)' }}>
                  Latest predicted close
                </p>
                <p className="font-serif tracking-tight"
                  style={{ fontSize: '2rem', color: 'var(--color-ink)', fontWeight: 700, lineHeight: 1 }}>
                  {symbol}{convertedClose}
                </p>
              </div>
        ...
        <div className="flex flex-col gap-5" style={{ width: '300px', flexShrink: 0 }}>
          ...
          <div className="border rounded-xl p-6"
            style={{ borderColor: 'var(--color-border)', background: 'var(--color-card-bg)' }}>
            <p className="text-[0.68rem] font-medium tracking-[0.1em] uppercase mb-3"
              style={{ color: 'var(--color-muted)' }}>
              How it works
            </p>
            <p className="text-[0.85rem] font-light leading-relaxed" style={{ color: 'var(--color-muted)' }}>
              {meta.description}
            </p>
          </div>
          ...
          <div className="border rounded-xl p-6"
            style={{ borderColor: 'var(--color-border)', background: 'var(--color-card-bg)' }}>
            <p className="text-[0.68rem] font-medium tracking-[0.1em] uppercase mb-4"
              style={{ color: 'var(--color-muted)' }}>
              Details
            </p>
            <div className="flex flex-col gap-4">
              {[
                ...
                { label: 'From', value: predictions[0] ? new Date(predictions[0].prediction_date).toLocaleDateString('en-GB') : '—' },
                { label: 'To',   value: latest ? new Date(latest.prediction_date).toLocaleDateString('en-GB') : '—' },
                { label: 'Complexity', value: '● '.repeat(meta.complexity) + '○ '.repeat(4 - meta.complexity) },
              ].map(({ label, value }) => (
                <div key={label}
                  className="flex justify-between items-center pb-3 border-b last:border-b-0 last:pb-0"
                  style={{ borderColor: 'var(--color-border)' }}>
                  <span className="text-[0.78rem] font-light" style={{ color: 'var(--color-muted)' }}>{label}</span>
                  <span className="text-[0.82rem] font-medium"
                    style={{ color: 'var(--color-ink)', fontFamily: label === 'Complexity' ? 'monospace' : 'inherit' }}>
                    {value}
                  </span>
                </div>
              ))}
          ...
          <div className="flex items-start gap-2.5 px-4 py-3 rounded-lg border"
            style={{ borderColor: 'var(--color-disclaimer-border)', background: 'var(--color-disclaimer-bg)' }}>
            <span style={{ color: 'var(--color-disclaimer-icon)', fontSize: '0.8rem', lineHeight: 1.4 }}>⚠</span>
            <p className="text-[0.7rem] font-light leading-relaxed" style={{ color: 'var(--color-disclaimer-text)' }}>
              Predictions are for{' '}
